...
function LocationSalle() {
  ...
  const [currentLocation, setCurrentLocation] = useState({ id_l: '', id_u: '', date_location: '', date_retour: '',id_s: '', couleur: '',});
...
const handleChangeComplete = (color) => {
  ...
  const fetchLocations = async () => {
    try {
      ...
      const formattedEvents = response.data.map((location) => ({
        id: location.id_l,
        // title: location.id_u,
        title: location.nom_utilisateur || 'Utilisateur inconnu',
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    const { id_u, date_location, date_retour, id_s, couleur } = currentLocation;
    ...
    // return;
      ...
      if (editMode) {
        await axios.put(`http://localhost:8085/location/salle/modifier/${currentLocation.id_l}`, { id_u, date_location, date_retour, id_s, couleur });
        Swal.fire('Modifié!', 'location modifiée avec succès.', 'success');
      } else {
        await axios.post('http://localhost:8085/location/salle/ajout', { id_u, date_location, date_retour, id_s, couleur });
        Swal.fire('Ajouté!', 'location ajoutée avec succès.', 'success');
      }

      setShowModal(false);
      setCurrentLocation({ id_l: '', id_u: '', date_location: '', date_retour: '' , id_s: '',});
      setEditMode(false);
      fetchLocations()
  ...
  const handleShowAdd = () => { setShowModal(true); setEditMode(false); setCurrentLocation({ id_l: '', id_u: '', date_location: '', date_retour: '', id_s: '' , couleur: '#07799E' }); };
  ...
  const handleDelete = async (id_l) => {
    ...
    if (result.isConfirmed) {
      try {
        await axios.delete(`http://localhost:8085/location/salle/supprimer/${id_l}`);
